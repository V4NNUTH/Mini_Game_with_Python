def getSevStr(number , minWidth = 0):
    #convert number to string in case it is an int or float
    ...
    for i ,numeral in enumerate(number):
        ...
        elif numeral == '-': #Render the negative sigh
            rows[0] += '    '
            rows[1] += ' __ '
            rows[2] += '    '
        ...
        #add a space(for the space in between numeals if this is not the last numeral)
        if i != len(number) -1:
            rows[0] += ' '
            rows[1] += ' '
            rows[2] += ' '
    return '\n'.join(rows)
#if this program is not imported display the numbers 00 to 99
if __name__ =="__main__":
    print("This module is meant to be  immported rather than run.")
